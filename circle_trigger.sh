...
echo "========"
curl -Ss -u "${CIRCLE_TOKEN}:" "${LAST_COMPLETED_BUILD_URL}"| jq -r 'map(select(.status == "success"))'
echo "========"
...
if  [[ ${LAST_COMPLETED_BUILD_SHA} == "null" ]]; then
  ...
  echo "2========"
  curl -Ss -u "${CIRCLE_TOKEN}:" "${LAST_COMPLETED_BUILD_URL}" \
      | jq -r "map(select(.status == \"success\"))"
  echo "========"
